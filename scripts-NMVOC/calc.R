...
dv.summ <- summarise(group_by(dv, exp, treat, comp, mass, elapsed.time), cum.mn = mean(cum), cum.sd = sd(cum))

colnames(dv.summ) <- c('exp', 'treat', 'compound', 'mass', 'elapsed.time', 'cum.mn', 'cum.sd')
