...
dat$treat <- dat$id
dat$treat <- mapvalues(dat$treat, from = c('1', '6', '9'), to = c(rep('Raw', 3)))
dat$treat <- mapvalues(dat$treat, from = c('2', '5', '10'), to = c(rep('Separated', 3)))
dat$treat <- mapvalues(dat$treat, from = c('4', '8', '12'), to = c(rep('NEO', 3)))
...
weather.22H <- dat[, c(1, 12)]
weather.22H$experiment <- '22H'
...
# DATA FRAMES
dat.22H <- dat[dat$elapsed.time == 161, ] # for statistics
dat.22H.end <- dat.summ[dat.summ$elapsed.time == 161, ] # cum emis
dat.22H.summ <- left_join(dat.summ, dat.fluxtime, by = 'elapsed.time') # flux

# # data frames for GitHub use
# write.csv(dat.22H, '../data-NH3/digA.A.csv')
# write.csv(dat.22H.end, '../data-NH3/digA.B.csv')
# write.csv(dat.22H.summ, '../data-NH3/digA.C.csv')
