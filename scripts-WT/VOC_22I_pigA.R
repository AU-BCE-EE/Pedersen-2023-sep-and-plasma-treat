...
# adding the valve no to TOF data
...
# Adding the time with lost data, so that flux corresponds with NH3 flux: 
dt$elapsed.time <- dt$elapsed.time + 1.77
...
dt$treat <- dt$id
dt$treat <- mapvalues(dt$treat, from = c('4', '6', '9'), to = c(rep('Raw', 3)))
dt$treat <- mapvalues(dt$treat, from = c('1', '5', '10'), to = c(rep('Separated', 3)))
dt$treat <- mapvalues(dt$treat, from = c('2', '8', '12'), to = c(rep('NEO', 3)))
