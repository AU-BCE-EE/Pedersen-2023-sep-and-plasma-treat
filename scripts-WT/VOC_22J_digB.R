...
dt$date.time <- dt$date.time + 60 * 2.1
...
dt$treat <- dt$id
dt$treat <- mapvalues(dt$treat, from = c('2', '5', '9'), to = c(rep('Raw', 3)))
dt$treat <- mapvalues(dt$treat, from = c('4', '6', '10'), to = c(rep('Separated', 3)))
dt$treat <- mapvalues(dt$treat, from = c('1', '8', '12'), to = c(rep('NEO', 3)))
...
dt <- dt[! dt$elapsed.time == 65.8, ]
dt <- dt[! dt$elapsed.time == 67.6, ]
